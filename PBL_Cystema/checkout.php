...
if (isset($_POST['order_btn'])) {
    ...
    if (mysqli_num_rows($cart_query) > 0) {
        ...
        if (mysqli_stmt_affected_rows($detail_query) > 0) {
            ...
            $activity_insert_query = mysqli_prepare($db, "INSERT INTO `user_activities` (user_email, activity_type, activity_description, timestamp) VALUES (?, ?, ?, CURRENT_TIMESTAMP)");
            mysqli_stmt_bind_param($activity_insert_query, "sss", $email, $activity_type, $activity_description);
...
$queryActivities = "SELECT * FROM user_activities WHERE user_email = ? ORDER BY timestamp ASC LIMIT 10";
